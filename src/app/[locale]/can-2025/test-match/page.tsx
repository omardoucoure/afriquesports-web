...
export default function TestMatchPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch formations (generated by autonomous agent)
        const formRes = await fetch('/api/can2025/match-formations?match_id=test-brazil-france-2006');
        const formJson = await formRes.json();
        setFormations(formJson.formations || null);
      } catch (error) {
  ...
  // Use formations from autonomous agent (if available)
  const homeFormation = formations?.home_team;
  const awayFormation = formations?.away_team;
          ...
          <div className="lg:col-span-1 space-y-6">
            {homeFormation ? (
              <>
                ...
                <div className="bg-white rounded-lg shadow-lg p-4">
                  ...
                  <div className="space-y-2">
                    {homeFormation.lineup.starters.map((player: any, idx: number) => (
                  ...
                  {homeFormation.lineup.coach && (
                    <div className="mt-3 pt-3 border-t text-center text-sm text-gray-600">
                      <span className="font-semibold">Coach:</span> {homeFormation.lineup.coach}
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white rounded-lg shadow-lg p-4">
                  ...
                  <div className="space-y-2">
                    {awayFormation.lineup.starters.map((player: any, idx: number) => (
                      <div key={idx} className="flex items-center gap-2 text-sm">
                  ...
                  {awayFormation.lineup.coach && (
                    <div className="mt-3 pt-3 border-t text-center text-sm text-gray-600">
                      <span className="font-semibold">Coach:</span> {awayFormation.lineup.coach}
                    </div>
                  )}
                </div>
              </>
              ...
              <div className="bg-white rounded-lg shadow-lg p-6 text-center text-gray-500">
                <div className="text-4xl mb-4">🔍</div>
                <p>L'agent autonome recherche les compositions d'équipe...</p>
                <p className="text-sm mt-2">Les formations apparaîtront automatiquement</p>
              </div>
            )}
          </div>
        </div>
