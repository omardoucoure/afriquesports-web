...
export async function GET() {
  try {
    ...
    // Add flag URLs to standings
    if (data.children) {
      data.children = data.children.map((group: any) => {
        if (group.standings?.entries) {
          group.standings.entries = group.standings.entries.map((entry: any) => {
    ...
    console.error('Error fetching CAN 2025 standings:', error);
