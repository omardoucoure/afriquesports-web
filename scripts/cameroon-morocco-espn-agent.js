...
function translateToFrench(event) {
  ...
  // First half begins
    ...
    type = 'kickoff';
    icon = '⚽';
    frenchText = `⚽ COUP D'ENVOI ! C'est parti entre le Cameroun et le Maroc !`;
  }
...
async function fetchAndProcessCommentary() {
  try {
    ...
    console.log(`\n⏱️  [${new Date().toLocaleTimeString()}] ${clock} | Cameroun ${homeScore} - ${awayScore} Maroc | Status: ${state}`);
...
// Main loop
async function main() {
  console.log('🎮 Cameroun vs Maroc - Live Commentary Agent');
